...
func setDefaultFromFlag(flagSchema *jsonschema.Schema, flag *pflag.Flag) {
	...
	// Nullable schemas generated by jsonschema-go use Types instead of Type.
	schemaType := flagSchema.Type
	for _, candidate := range flagSchema.Types {
		if candidate == "null" {
			continue
		}
		if schemaType != "" {
...
func parseObject(defValue string, schema *jsonschema.Schema) any {
	...
	if !strings.HasPrefix(defValue, "[") || !strings.HasSuffix(defValue, "]") {
		slog.Warn("malformed object default value: must start with '[' and end with ']'", "value", defValue)
		return nil
	}
	if schema == nil {
